...
case ${osVersion,,} in
  ...
  *"8."*)
    ...
    yum install -y autoconf automake bison bzip2 \
                   dejavu-fonts-common dejavu-sans-fonts dejavu-serif-fonts \
                   flex fontconfig-devel freetype-devel gcc gcc-c++ \
                   libaio libfontenc libicu-devel libjpeg-devel libmcrypt-devel \
                   libpng-devel libtool libxml2-devel make net-tools openssl-devel \
                   pcre-devel postgresql-devel psmisc re2c readline-devel sqlite-devel \
                   unixODBC-devel wget xorg-x11-font-utils libaio libaio-devel
